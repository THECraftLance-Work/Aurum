    ...
    {open && <div className="search-panel-enter fixed left-3 right-3 top-14 z-50 sm:absolute sm:left-0 sm:right-0 sm:top-0" role="dialog" aria-modal="false" aria-label="Global search">
      <div className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_80px_-24px_rgba(15,23,42,0.45)]">
        ...
        <div className="max-h-[min(30rem,68vh)] overflow-y-auto p-2">
          ...
          {query.trim() && !loading && !results.length && <div className="px-3 py-10 text-center"><Search className="mx-auto mb-2 h-7 w-7 text-slate-300" /><p className="text-sm font-medium text-slate-700">No results found</p><p className="mt-1 text-xs text-slate-400">Try searching for a project (Tatva, Aurum, Sri Varaha), booking ID, or customer name.</p></div>}
