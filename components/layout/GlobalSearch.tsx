...
const labels: Record<Result["kind"], string> = { project: "Projects", booking: "Bookings", customer: "Customers", user: "Employees" };
...
export default function GlobalSearch({ user }: { user: SessionUser }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      projects.data?.filter((p: any) => p.slug !== "sri-varaha").forEach((p: any) => next.push({
        ...
        id: p.id,
        title: p.name,
        subtitle: `Switch workspace to ${p.name} dashboard`,
        href: "/dashboard",
        projectId: p.id
      }));
  ...
  function keyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "Enter" && selectable[active]) { e.preventDefault(); go(selectable[active]); }
  }
  ...
  return <div ref={searchRef} className="relative">
    ...
    {open && <div className="search-panel-enter fixed left-3 right-3 top-14 z-50 sm:absolute sm:left-0 sm:right-0 sm:top-0" role="dialog" aria-modal="false" aria-label="Global search">
      <div className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_80px_-24px_rgba(15,23,42,0.45)]">
        <div className="flex items-center gap-3 border-b border-border px-4 py-3">
          ...
          <input ref={inputRef} autoFocus value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={keyDown} placeholder="Search bookings, customers, projects, users…" aria-label="Search" className="min-w-0 flex-1 bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400" />
        ...
        <div className="max-h-[min(30rem,68vh)] overflow-y-auto p-2">
          ...
          {query.trim() && !loading && !results.length && <div className="px-3 py-10 text-center"><Search className="mx-auto mb-2 h-7 w-7 text-slate-300" /><p className="text-sm font-medium text-slate-700">No results found</p><p className="mt-1 text-xs text-slate-400">Try searching for a project (Tatva, Aurum, Sree Varaaha), booking ID, or customer name.</p></div>}
          {query.trim() && !loading && groups.map((group) => <section key={group.kind} className="mb-2 last:mb-0"><p className="px-3 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400">{labels[group.kind]}</p>{group.items.map((r) => { const index = results.indexOf(r); return <button key={`${r.kind}-${r.id}`} type="button" onClick={() => go(r)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors ${index === active ? "bg-slate-100" : "hover:bg-slate-50"}`}><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-slate-100 text-slate-500">{r.kind === "project" ? <CreditCard className="h-4 w-4 text-[#ec3013]" /> : r.kind === "booking" ? <FileText className="h-4 w-4" /> : r.kind === "customer" ? <UserRound className="h-4 w-4" /> : <UsersRound className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-slate-800">{r.title}</span>{r.subtitle && <span className="block truncate text-xs text-slate-400">{r.subtitle}</span>}</span>{r.kind === "project" && <span className="text-[10px] uppercase font-bold text-white bg-slate-900 px-2 py-0.5 rounded">Switch</span>}<ArrowRight className="h-4 w-4 text-slate-300" /></button>; })}</section>)}
        </div>
