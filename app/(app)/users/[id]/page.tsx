...
export default async function DirectorUserDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const safeTickets = (rawTickets ?? []).map((t: any) => ({ ...t, comments: commentsMap.get(t.id) ?? [], assignee: null as any }));
  ...
  for (const t of safeTickets) {
    ...
    if (raw?.assigned_to) (t as any).assignee = assigneeMap.get(raw.assigned_to) ?? null;
  }

  const safeUserDocs = (userDocs ?? []).map((a: any) => ({ ...a, uploader: Array.isArray(a.uploader) ? a.uploader[0] ?? null : a.uploader ?? null }));
  const docUrls: DocUrl[] = safeUserDocs.map((a: any) => ({ id: a.id, file_name: a.file_name, file_size: a.file_size, mime_type: a.mime_type, label: a.label, entity_type: a.entity_type, entity_id: a.entity_id, created_at: a.created_at, uploader: a.uploader }));

  return <UserDetailClient user={u as any} bookings={safeBookings as any} tickets={safeTickets as any} docUrls={docUrls} embedded />;
}
