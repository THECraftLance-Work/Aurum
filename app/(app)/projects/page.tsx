  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[parent, ...children, ...orphans].filter(Boolean).map((p: any, i: number) => {
          const isParent = p.slug === "sri-varaha";
          const grad = isParent ? "from-violet-600 via-indigo-600 to-blue-600" : i % 3 === 1 ? "from-emerald-500 via-teal-500 to-cyan-500" : i % 3 === 2 ? "from-orange-500 via-red-500 to-pink-500" : "from-slate-800 via-slate-700 to-slate-600";
          return (
            <div key={p.id} className={`rounded-2xl bg-gradient-to-br ${grad} p-[1px]`}>
              <div className="rounded-[15px] bg-white p-5 h-full flex flex-col">
                <div className="flex items-center gap-2">
                  <span className={`grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br ${grad} text-white text-xs font-bold`}>{p.name.slice(0,2).toUpperCase()}</span>
                  <span className="text-sm font-semibold truncate">{p.name}</span>
                  {isParent && <span className="ml-auto rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700">PARENT</span>}
                </div>
                <div className="mt-1 text-xs text-slate-500">/{p.slug} • {p.is_active ? "Active" : "Archived"}</div>
                <div className="mt-auto pt-4">
                  <span className="text-xs text-slate-400">{isParent ? "Houses Aurum, Tatva..." : "Isolated bookings & payments"}</span>
                </div>
              </div>
            </div>
          );
