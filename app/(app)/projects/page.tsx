  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[...children, ...orphans].filter(Boolean).map((p: any, i: number) => {
          const grad = i % 3 === 0 ? "from-slate-800 via-slate-700 to-slate-600" : i % 3 === 1 ? "from-emerald-500 via-teal-500 to-cyan-500" : "from-orange-500 via-red-500 to-pink-500";
          return (
            <div key={p.id} className={`rounded-2xl bg-gradient-to-br ${grad} p-[1px]`}>
              <div className="rounded-[15px] bg-white p-5 h-full flex flex-col">
                ...
                <div className="mt-auto pt-4">
                  <span className="text-xs text-slate-400">Isolated bookings &amp; payments</span>
                </div>
              </div>
            </div>
          );
